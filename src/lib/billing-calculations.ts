...
export function calculateBillFromTariff(
    ...
    const roundedBase = parseFloat(baseWaterCharge.toFixed(2));
    const roundedMaintenance = parseFloat(maintenanceFee.toFixed(2));
    const roundedSanitation = parseFloat(sanitationFee.toFixed(2));
    const roundedVat = parseFloat(vatAmount.toFixed(2));
    const roundedMeterRent = parseFloat(meterRent.toFixed(2));
    const roundedSewerage = parseFloat(sewerageCharge.toFixed(2));
    const roundedAdditional = parseFloat(additionalFeesTotal.toFixed(2));

    const totalBill = roundedBase + roundedMaintenance + roundedSanitation + roundedVat + roundedMeterRent + roundedSewerage + roundedAdditional;

    return {
        totalBill: parseFloat(totalBill.toFixed(2)),
        baseWaterCharge: roundedBase,
        maintenanceFee: roundedMaintenance,
        sanitationFee: roundedSanitation,
