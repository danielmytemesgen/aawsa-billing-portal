        ...
        <div className="w-full py-6">
            <div className="flex items-center justify-between w-full relative">
                ...
                {steps.map((step, index) => {
                    ...
                    // Highlight the Pending step in red when bill was rejected back to Rework
                    const isRejectionPoint = currentStatus === 'Rework' && index === 1;
                        ...
                        <div key={step.status} className="flex flex-col items-center bg-white px-2">
                            <div
                                className={cn(
                                    ...
                                    isRejectionPoint ? "bg-red-500 border-red-500 text-white" :
                                    isCompleted ? "bg-green-500 border-green-500 text-white" :
                                        isCurrent ? "bg-blue-500 border-blue-500 text-white" :
                                            "bg-white border-gray-300 text-gray-400"
                                )}
                            >
                                {isRejectionPoint ? <XCircle size={20} /> : <step.icon size={20} />}
                            </div>
                            <span className={cn(
                                ...
                                {isRejectionPoint ? "Rejected" : step.label}
                            </span>
                        </div>
                    );
                })}
