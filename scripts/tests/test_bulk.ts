...
async function testMeter(bulkMeterId: string) {
    ...
    const associatedCustomers = customers.filter((c: any) => c.assigned_bulk_meter_id === bulkMeterId);

    const bmUsage = (bulkMeter.current_reading ?? 0) - (bulkMeter.previous_reading ?? 0);
    const totalIndivUsage = associatedCustomers.reduce((sum: number, cust: any) => sum + ((cust.current_reading ?? 0) - (cust.previous_reading ?? 0)), 0);
