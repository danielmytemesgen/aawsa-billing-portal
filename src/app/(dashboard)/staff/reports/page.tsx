...
const availableStaffReports: ReportType[] = [
  ...
  },
  {
    ...
    description: "Monthly summary of billing amounts broken down and grouped by Branch and GL Code (WNDGL, WDGL, WBORGL, SNTGL, SEWERGL, WPFNTGL, MTRRNTGL, MNTGL, FIREGL). Shows COUNT, TOTAL_AMOUNT, VAT_AMOUNT and WITH_OUT_VAT per GL code per period.",
    category: 'Finance',
    ...
    getData: async (filters) => {
      ...
      const bulkMeters: any[] = (bulkMetersRes?.data ?? []).map((bm: any) => ({ ...bm, branchId: bm.branch_id || bm.branchId, customerKeyNumber: bm.customerKeyNumber, chargeGroup: bm.charge_group || bm.chargeGroup }));
      const customers: any[]  = (customersRes?.data ?? []).map((c: any) => ({ ...c, branchId: c.branch_id || c.branchId, customerKeyNumber: c.customerKeyNumber, customerType: c.customer_type || c.customerType }));
      const branches: any[]   = branchesRes?.data ?? [];
      ...
      if (branchId) allBills = allBills.filter((b: any) => getBillMeta(b).branchId === branchId);
      if (startDate && endDate) {
        const start = startDate.getTime(), end = endDate.getTime();
        allBills = allBills.filter((b: any) => { try { const d = new Date(b.bill_period_end_date || b.created_at); return d.getTime() >= start && d.getTime() <= end; } catch { return false; } });
      }
      ...
      const grouped: Record<string, { count: number; branch: string; glCode: string; period: string; totalAmount: number; vatAmount: number; withoutVat: number; }> = {};
      ...
      for (const b of allBills) {
        ...
        if (cgLower.includes('borehole')) baseWaterGlCode = 'WBORGL';
        else if (cgLower.includes('fountain')) baseWaterGlCode = 'WPFNTGL';
        else if (cgLower.includes('domestic') && !cgLower.includes('non')) baseWaterGlCode = 'WDGL';
        ...
        const addItem = (glCode: string, amount: number, vatAmt: number = 0) => {
          ...
          if (!grouped[key]) grouped[key] = { count: 0, branch: branchCode, glCode, totalAmount: 0, vatAmount: 0, withoutVat: 0, period: periodStr };
          grouped[key].count += 1;
