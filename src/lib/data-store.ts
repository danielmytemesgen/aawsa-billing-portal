...
import { calculateBillFromTariff, type CustomerType, type SewerageConnection, type PaymentStatus, type BillCalculationResult, type TariffInfo, type TariffTier } from '@/lib/billing-calculations';
import { getEndDayOfMonth, normalizeTariff } from '@/lib/billing-utils';
...
async function computeBillLocal(usage: number, customerType: CustomerType, sewerageConnection: SewerageConnection, meterSize: number, month: string, sewerageCONS?: number): Promise<BillCalculationResult> {
  ...
  try {
    ...
    // Find the most recent tariff with effective_date <= the actual last day of the billing month.
    // Previously used '-31' which is not a valid day for shorter months; now uses the real last day.
    const readingDate = getEndDayOfMonth(month);
    ...
    // Use normalizeTariff so defaults are applied identically to the server path in billing.ts
    const tariffConfig: TariffInfo = normalizeTariff(tariffRow);
...
): Promise<StoreOperationResult<{ count: number; insertedCount?: number; updatedCount?: number; rowResults?: Array<{ custKey: string; success: boolean; error?: string }> }>> => {
  if (!items || items.length === 0) return { success: true, data: { count: 0, insertedCount: 0, updatedCount: 0, rowResults: [] } };
