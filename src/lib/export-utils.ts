...
export const buildBillManagementAuditRows = (
  bills: DomainBill[],
  branches: Branch[] = []
) => {
  return bills.map((bill: any) => {
    const billKey = getBillKey(bill);
    const customerName = bill.CUSTOMERNAME || bill.customerName || bill.customer_name || bill.name || "N/A";
    const customerKey = getCustomerKeyDisplay(bill);
    const branchName = getBranchNameDisplay(bill, [], [], branches);
    const monthValue = bill.monthYear || bill.month_year || "-";
    const createdAt = bill.createdAt || bill.created_at || bill.created || "-";
    const updatedAt = bill.updatedAt || bill.updated_at || bill.updated || "-";
    const amountPaid = parseNumber(bill.amountPaid ?? bill.amount_paid ?? bill.AMOUNTPAID ?? 0) ?? 0;
    const currentBill = getMonthlyBillAmt(bill);
    const outstanding = parseNumber(bill.OUTSTANDINGAMT) ?? 0;
    ...
    const status = String(bill.status || "Unknown");
    const paymentStatus = String(bill.paymentStatus || bill.payment_status || (amountPaid > 0 ? "Paid" : "Unpaid"));
    const dueDate = formatDate(bill.dueDate || (bill as any).due_date);
    const dueDateValue = bill.dueDate || (bill as any).due_date;
    const overdue = Boolean(dueDateValue && isBefore(new Date(dueDateValue), new Date()));
    ...
    const agingBucket = d60 > 0.01 ? "60+" : (d30_60 > 0.01 ? "30-60" : (d30 > 0.01 ? "30" : "Current"));
    ...
    return {
      ...
      "Amount Paid (ETB)": amountPaid.toFixed(2),
...
export const exportBillManagementAuditToCsv = (
  bills: DomainBill[],
  branches: Branch[] = [],
  filenamePrefix = "bill_management_audit_report"
) => {
  const headers = [
    "Bill Key",
    "Bill ID",
  ...
  const data = buildBillManagementAuditRows(bills, branches);
  const blob = arrayToCsvBlob(data, headers);
  const dateStr = format(new Date(), "yyyy-MM-dd");
  downloadFile(blob, `${filenamePrefix}_${dateStr}.csv`);
};

export const exportBillManagementAuditToXlsx = (
  bills: DomainBill[],
  branches: Branch[] = [],
  filenamePrefix = "bill_management_audit_report"
) => {
