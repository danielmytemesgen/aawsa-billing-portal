              ...
              <div className="print-body">
                <div className="print-section">
                  ...
                  <table className="print-table">
                    <tbody>
                      <tr><td>Bulk meter name:</td><td>{bulkMeter.name}</td></tr>
                      <tr><td>Customer key number:</td><td>{bulkMeter.customerKeyNumber}</td></tr>
                      <tr><td>Contract No:</td><td>{bulkMeter.contractNumber ?? 'N/A'}</td></tr>
                      <tr><td>Branch:</td><td>{displayBranchName ?? 'N/A'}</td></tr>
                      <tr><td>Sub-City:</td><td>{bulkMeter.location}</td></tr>
                    </tbody>
                  </table>
                </div>
                ...
                <div className="print-section">
                  ...
                  <table className="print-table">
                    <tbody>
                      <tr><td>Bulk Meter Category:</td><td>{bulkMeter.chargeGroup}</td></tr>
                    ...
                    <div key={bill.id ?? `${bill.monthYear}-${String(bill.billPeriodEndDate)}-${_billIndex}`} className="border-b p-4">
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-2 text-sm">
                        ...
                        {(() => {
                          ...
                          const getUnpaid = (b: any) => b.paymentStatus === 'Paid' ? 0 : Number(b.TOTALBILLAMOUNT) - Number(b.amountPaid || 0);

                          if (fullIndex < billingHistory.length - 1) d30 = getUnpaid(billingHistory[fullIndex + 1]);
                          if (fullIndex < billingHistory.length - 2) d60 = getUnpaid(billingHistory[fullIndex + 2]);
